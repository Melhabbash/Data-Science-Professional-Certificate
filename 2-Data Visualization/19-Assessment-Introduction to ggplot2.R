...
# For the murders data, we plotted total murders versus population sizes in the 
# videos.

# Explore the murders data frame to remind yourself of the names for the
# two variables (total murders and population size) we want to plot and select
# the correct answer.
str(murders)
...
# making sure we enter the variable names in the desired order:

murders %>% ggplot(aes(population, total)) +
  geom_point()

# Remake the plot but flip the axes so that total is on the x-axis and
...
# will give us the error message: Error: geom_label requires the following 
# missing aesthetics: label
...
# We previously used this code to make a plot using the state abbreviations as 
# labels:
...
# Now we are going to change the axes to log scales to account for the fact that
# the population distribution is skewed. Let's start by defining an object p that 
# holds the plot we have made up to now:
...
p + scale_x_log10()
# Change both axes to be in the log scale on a single graph. Make sure you do not
# redefine p - just add the appropriate layers.
p <- murders %>% ggplot(aes(population, total, label = abb, color = region)) + 
  geom_label()
...
# We are now going to add a title to this plot. We will do this by adding yet 
# another layer, this time with the function ggtitle.
# Edit the code above to add the title "Gun murder data" to the plot.
  
# #######################
# Exercise 15. Histograms
# We are going to shift our focus from the murders dataset to explore the heights 
# dataset.
# We use the geom_histogram function to make a histogram of the heights in the 
# heights data frame. When reading the documentation for this function we see 
...
# We are now going to make a histogram of the heights so we will load the heights 
# dataset. The following code has been pre-run for you to load the heights 
# dataset:
...
# geom_histogram function to make the histogram.
...
# Note that when we run the code from the previous exercise we get the following
# warning:
...
# In this case, we will not make an object p. Instead we will render the plot 
# using a single line of code. In the previous exercise, we could have created
# a histogram using one line of code like this:
  
  heights %>% ggplot(aes(height)) +  geom_histogram()
# Now instead of geom_histogram we will use geom_density to create a smooth 
# density plot.
heights %>% 
...
# Because each point will be assigned to a different density depending on 
...
# In the previous exercise we made the two density plots, one for each sex, 
# using:
  
  heights %>% 
  ggplot(aes(height, group = sex)) + 
  geom_density()
# We can also assign groups through the color or fill argument. For example, 
# if you type color = sex ggplot knows you want a different color for each sex.
# So two densities must be drawn. You can therefore skip the 
# group = sex mapping. Using color has the added benefit that it uses color to 
...
# geom_density geometry, color creates a colored line for the smooth 
...
# Set the alpha parameter to 0.2 in the geom_density function to make this 
# change.
