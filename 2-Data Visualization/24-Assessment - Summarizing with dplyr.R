...
# by the United States National Center for Health Statistics (NCHS). This center has 
# conducted a series of health and nutrition surveys since the 1960's.
...
# Part of this dataset is made available via the NHANES package which can be loaded 
# this way:
...
# females. Note that the category is coded with 20-29, with a space in front of 
# the 20! The AgeDecade is a categorical variable with these ages.
...
# Use the pipe to apply the function filter, with the appropriate logicals, to
# NHANES.
# Remember that this age group is coded with 20-29, which includes a space. 
# You can use head to explore the NHANES table to construct the correct call to 
# filter.
...
# systolic blood pressure as average and standard_deviation to a variable called 
# ref.
...
# Modify the line of sample code to assign the average to a numeric variable 
# called ref_avg using the . or pull.
...
# Let's continue practicing by calculating two other data summaries: the minimum 
# and the maximum.
# Again we will do it for the BPSysAve variable and the group of 20-29 year old 
# females.
    
# Report the min and max values for the same group as in the previous exercises.
# Use filter and summarize connected by the pipe %>% again. The functions min 
# and max can be used to get the values you want.
# Within summarize, save the min and max of systolic blood pressure as minbp 
# and maxbp.
...
# you will split a data table into groups and then compute summary statistics 
# for each group.
# We w ill compute the average and standard deviation of systolic blood pressure
# for females for each age group separately. Remember that the age groups are 
# contained in AgeDecade.
# Use the functions filter, group_by, summarize, and the pipe %>% to compute the
# average and standard deviation of systolic blood pressure for females for each 
# age group separately.
# Within summarize, save the average and standard deviation of systolic blood 
# pressure (BPSysAve) as average and standard_deviation.
# Note: ignore warnings about implicit NAs. This warning will not prevent your 
...
# Calculate the average and standard deviation of systolic blood pressure for
# males for each age group separately using the same methods as in the previous 
# exercise.
# Note: ignore warnings about implicit NAs. This warning will not prevent your 
# code from running or being graded correctly. 
  library(dplyr)
...
# systolic blood pressure using group_by(AgeDecade, Gender).
...
# We will learn to use the arrange function to order the outcome acording to one
# variable.
...
# In this example, we will compare systolic blood pressure across values of the 
# Race1 variable for males between the ages of 40-49.

# Compute the average and standard deviation for each value of Race1 for males 
# in the age decade 40-49.
# Order the resulting table from lowest to highest average systolic blood 
# pressure.
# Use the functions filter, group_by, summarize, arrange, and the pipe %>% to do
# this in one line of code.
# Within summarize, save the average and standard deviation of systolic blood 
# pressure as average and standard_deviation.
  library(dplyr)
  library(NHANES)
  data(NHANES)
