...
# An American roulette wheel has 18 red, 18 black, and 2 green pockets. Each red and black 
# pocket is associated with a number from 1 to 36. The two remaining green slots 
# feature "0" and "00". Players place bets on which pocket they think a ball will land in 
# after the wheel is spun. Players can bet on a specific number (0, 00, 1-36) or color 
# (red, black, or green).
# 
...
# In American roulette, the payout for winning on green is $17. This means that if you 
# bet $1 and it lands on green, you get $17 as a prize.
# 
# Use the sample function return a random value from a specified range of values.
# Use the prob = argument in the sample function to specify a vector of probabilities for 
...
# Sample one time.
X <- mean(sample(c(17,-1), 1, replace = TRUE, prob = c(p_green, p_not_green)))    
...
# In American roulette, the payout for winning on green is $17. This means that if you
# bet $1 and it lands on green, you get $17 as a prize.In the previous exercise, you 
...
# Calculate the expected outcome if you win $17 if the ball lands on green and you 
# lose $1 if the ball doesn't land on green
...
# variable and its expected value. You calculated a random variable X in exercise 2 and 
# the expected value of that random variable in exercise 3.
# 
# Now, compute the standard error of that random variable, which represents a single 
# outcome after one spin of the roulette wheel.
...
# 
# Now create a random variable S that sums your winnings after betting on green 1,000 
# times.
...
# Be sure to assign a probability to each outcome and to indicate that you are sampling
# with replacement.
...
# In the previous exercise, you generated a vector of random outcomes, S, after betting 
# on green 1,000 times.
# 
# What is the expected value of S?
# 
# Using the chances of winning $17 (p_green) and the chances of losing $1 (p_not_green),
# calculate the expected outcome of a bet that the ball will land in a green pocket over 
# 1,000 bets.
...
# You generated the expected value of S, the outcomes of 1,000 bets that the ball lands 
# in the green pocket, in the previous exercise.
# 
...
sqrt(n)*abs(-1-17)*sqrt(p_not_green*p_green)
